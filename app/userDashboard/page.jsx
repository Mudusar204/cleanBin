...
const Page = () => {
  ...
  const skipThisWeekFun = async () => {
    try {
      ...
      let res = await dispatch(skipThisWeek(moment(weekDate, 'YYYY-MM-DD').format('M/D/YYYY')))
  ...
  useEffect(() => {
    const getUserDetail = async () => {
      try {
        ...
        const tomorrowCleaning = getTomorrowDateObject(user?.payload?.data?.cleanings)
        const previousCleaning = getPreviousDateObject(user?.payload?.data?.cleanings)
        console.log('🚀 ~ getUserDetail ~ previousCleaning:', previousCleaning)
        const cleaningHistory = getCleaningsBeforeCurrentDate(user?.payload?.data?.cleanings)
        ...
        <div className='w-full mx-[50px] max-sm:mx-0 h-screen overflow-scroll'>
          ...
          <div className='flex justify-between flex-wrap mt-[30px]'>
            <CleaningCard
              ...
              w={'50%'}
            />
            <CleaningCard item={{ ...tomorrowCleaningState, title: 'Next Cleaning' }} w={'50%'} />
          </div>
